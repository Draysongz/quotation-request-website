...
$user = "root"; // User 
$password = ""; // Password 
$dbname = "quotationrequest"; // Database name
...
$display_heading = array('ref_no'=> 'Reference', 'descriptions'=> 'Description', 'code'=> 'Code', 'quantity'=> 'Quantity');
 
$result = mysqli_query($conn, "SELECT ref_no, descriptions, code, quantity FROM quote_items WHERE ref_no = ".$ref_no) or die("Oop's! Something went wrong. Please contact the support team for help"); //die("database error:". mysqli_error($conn));
$header = mysqli_query($conn, "SHOW columns FROM quote_items WHERE field != 'date_added'");
